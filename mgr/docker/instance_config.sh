...
if [ $REDIS_HOST ]; then
    sed -i "s|REDIS_HOST.*|REDIS_HOST = '$REDIS_HOST'|" vfc/gvnfm/vnfmgr/mgr/mgr/pub/config/config.py
fi

if [ $SDC_BASE_URL ]; then
    sed -i "s|SDC_BASE_URL.*|SDC_BASE_URL = '$SDC_BASE_URL'|" vfc/gvnfm/vnfmgr/mgr/mgr/pub/config/config.py
fi

sed -i "s|DB_NAME.*|DB_NAME = 'vfcnfvolcm'|" vfc/gvnfm/vnfmgr/mgr/mgr/pub/config/config.py
sed -i "s|DB_USER.*|DB_USER = 'vfcnfvolcm'|" vfc/gvnfm/vnfmgr/mgr/mgr/pub/config/config.py
sed -i "s|DB_PASSWD.*|DB_PASSWD = 'vfcnfvolcm'|" vfc/gvnfm/vnfmgr/mgr/mgr/pub/config/config.py
